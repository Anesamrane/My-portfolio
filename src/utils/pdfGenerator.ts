...
export function generateResumePDF() {
  ...
  // Helper to add lines with auto pagination
  const checkPageOverflow = (heightNeeded: number) => {
    if (y + heightNeeded > 280) {
      doc.addPage();
      y = 15; // Reset to top margin on new page
    }
  };
  ...
  skillCategories.forEach(skill => {
    ...
    // Calculate width of bold label to position the description correctly
